...
def display(image_rss, kspace_rss, x_slice, y_slice, z_slice, maskA, maskB, gap):
    ...
    plt.imshow(maskA[:, :, z_slice], alpha = 0.3, cmap = 'Greens')
    ...
    plt.imshow(maskA[:, y_slice, :], alpha = 0.3, cmap = 'Greens')
    ...
    plt.imshow(maskA[x_slice, :, :], alpha = 0.3, cmap = 'Greens')

    # plt.imshow(maskA[x_slice, :, :], alpha = 0.3, cmap = 'Reds')



    # name = f'signal retention{data_id}, x={z_slice}, y={y_slice}, z={x_slice}, gap={gap}.png'
    # path = os.path.join(r"C:\Users\selin\MRI defacer\results", name)
    # plt.savefig(path)
